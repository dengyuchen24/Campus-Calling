...
namespace dyc
{
	Character* ActionQueue::top()
	{
		if (queue.empty())
		{
			LOG_COUT("[WARNING] Action queue is empty!");
			return nullptr;
		}
		return queue[0];
	}
	...
	GameWnd::GameWnd(WndManager* m) : sfWindow(m)
	{
		...
		if (!cai2) {
			LOG_CERR("cai2 is nullptr!");
		}
		else {
			LOG_COUT("Success get CaiShuYou object, ID: " << cai2->GetID());
